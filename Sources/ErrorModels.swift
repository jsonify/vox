...
public enum VoxError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .apiKeyMissing(let service):
            return "❌ API key missing for \(service)\n" +
                   ...
                   "2) Set \(service.uppercased())_API_KEY environment variable, " +
                   "3) Or use native transcription without --force-cloud."
    ...
    private var componentName: String {
        ...
        case .processingFailed:
            return "Processor"
        case .transcriptionInProgress, .speechRecognitionUnavailable, 
             .invalidAudioFile:
            return "Transcription"
        case .insufficientDiskSpace, .invalidOutputPath, .permissionDenied, 
             .backupFailed, .atomicWriteFailed, .pathCreationFailed:
            return "OutputWriter"
        }
    }
}
