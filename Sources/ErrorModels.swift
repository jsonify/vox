...
public enum VoxError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .audioExtractionFailed(let reason):
            return "❌ Audio extraction failed: \(reason)\n   💡 Try: 1) Check if file is corrupted, 2) Ensure enough disk space, 3) Verify file permissions."
        case .transcriptionFailed(let reason):
            return "❌ Transcription failed: \(reason)\n   💡 Try: 1) Use --force-cloud for cloud transcription, 2) Specify --language for better accuracy, 3) Check your internet connection."
        case .outputWriteFailed(let reason):
            return "❌ Failed to write output: \(reason)\n   💡 Check: 1) Output directory exists, 2) You have write permissions, 3) Enough disk space available."
        case .apiKeyMissing(let service):
            return "❌ API key missing for \(service)\n   💡 Set your API key: 1) Use --api-key flag, 2) Set \(service.uppercased())_API_KEY environment variable, 3) Or use native transcription without --force-cloud."
        ...
        case .temporaryFileCreationFailed(let reason):
            return "❌ Failed to create temporary file: \(reason)\n   💡 Check: 1) Disk space in /tmp, 2) System permissions, 3) Restart if the issue persists."
        ...
        case .processingFailed(let reason):
            return "❌ Processing failed: \(reason)\n   💡 Try: 1) Run with --verbose for details, 2) Check file integrity, 3) Ensure sufficient system resources."
        case .transcriptionInProgress:
            return "❌ A transcription is already in progress\n   💡 Wait for the current transcription to complete or restart the application."
        case .speechRecognitionUnavailable:
            return "❌ Speech recognition is not available\n   💡 Try: 1) Use --force-cloud for cloud transcription, 2) Check macOS Speech Recognition settings, 3) Ensure macOS 12.0+ is installed."
        case .invalidAudioFile:
        ...
        case .invalidOutputPath(let path):
            return "❌ Invalid output path: \(path)\n   💡 Check: 1) Directory exists, 2) Path is writable, 3) Use absolute or relative paths correctly."
        case .permissionDenied(let path):
            return "❌ Permission denied: \(path)\n   💡 Fix: 1) Check file/directory permissions, 2) Use 'sudo' if needed, 3) Ensure you own the target directory."
